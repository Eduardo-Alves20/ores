...
function isAdmin(req) {
  return req?.session?.user?.perfil === PERFIS.ADMIN;
}
...
class ContaController {
    ...
    try {
      ...
      const payload = admin
        ? {
            ...
            tipoCadastro: String(req.body?.tipoCadastro || "").trim(),
            ativo: String(req.body?.ativo || "").trim() === "true",
          }
        : {
            email: String(req.body?.email || "").trim(),
            telefone: String(req.body?.telefone || "").trim(),
  ...
  static async notificacoes(req, res) {
    try {
      ...
      const defaultLimit = perfil === PERFIS.ADMIN ? 60 : 40;
