...
const { AgendaEvento, TIPOS_AGENDA } = require("../schemas/AgendaEvento");
const { PERFIS } = require("../config/roles");
const { registrarAuditoria } = require("../services/auditService");

const PERFIS_COM_VISAO_GLOBAL = new Set([PERFIS.ADMIN, PERFIS.ATENDENTE]);
const PERFIS_PERMITIDOS_NA_AGENDA = new Set([PERFIS.ADMIN, PERFIS.ATENDENTE, PERFIS.TECNICO]);
...
function canViewAll(user) {
  return PERFIS_COM_VISAO_GLOBAL.has(user?.perfil);
}
...
class AgendaController {
  static async listar(req, res) {
    try {
      ...
      if (!user || !PERFIS_PERMITIDOS_NA_AGENDA.has(user.perfil)) {
  ...
  static async listarProfissionais(req, res) {
    try {
      ...
      const profissionais = await Usuario.find({
        ativo: true,
        perfil: { $in: [PERFIS.ADMIN, PERFIS.ATENDENTE, PERFIS.TECNICO] },
      })
